function [m,L] = knndsval(xtrain,ytrain,K,gamm,alpha,loo,xtest);
...
function m = classdstst(alpha,gamm,xtst,ds,is,ytrain,K);
...
for i=1:N,
   for j=1:K,
     m1 = zeros(M+1,1);
     m1(ytrain(is(j,i))) = alpha*exp(-gamm(ytrain(is(j,i))).^2*ds(j,i));
